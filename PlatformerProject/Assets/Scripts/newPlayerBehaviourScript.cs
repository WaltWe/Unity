...
public class newPlayerBehaviourScript : MonoBehaviour {
    ...
    private void win()
    {
        gameObject.AddComponent<TextMesh>().text = "You Win!";
    }

    private void Update()
    {
        if (playerdata.numEnemies == 0 && Time.time >= 30)
        {
            win();
        }
        powerUpBehaviour();
    ...
    {
        int mult = 1;
        if (playerdata.gravity.y + playerdata.gravity.x > 0) mult = -1;
        if (Mathf.Abs(Input.GetAxis("Horizontal")) > 0.01)
        {
            this.GetComponent<Rigidbody2D>().velocity += Vector2.Perpendicular(new Vector2(playerdata.gravity.x * Input.GetAxis("Horizontal") * 10 * mult * getNegative(playerdata.gravity.x), playerdata.gravity.y * Input.GetAxis("Horizontal") * 10 * mult * getNegative(playerdata.gravity.x)));
        }
        else
        {
            this.GetComponent<Rigidbody2D>().velocity -= Vector2.Perpendicular(new Vector2(playerdata.gravity.x * 10 * mult, playerdata.gravity.y * Input.GetAxis("Horizontal") * 10 * mult));
        }
    }
    void gravity()
    {
