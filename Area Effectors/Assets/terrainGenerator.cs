...
public class terrainGenerator : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            GameObject curGround = Instantiate(ground, new Vector2((i + groundWidth/2), -1.75f), new Quaternion(0, 0, 0, 0));
    ...
    void Update()
    {
        ...
        if (curX - X <= -2 * leftX)
        {
            GameObject curGround = Instantiate(ground, new Vector2((curX + groundWidth + groundWidth/2), curY + Random.Range(-100f, 100f) / 250f), new Quaternion(0, 0, 0, 0));
            curX = curGround.transform.position.x;
            curY = curGround.transform.position.y;
        }
    }
